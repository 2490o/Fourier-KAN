...
class ACVCCorruption:
    """
    Apply ACVC corruptions (blur, noise, etc.) to the image.
    """
    def __init__(self, p=0.5):
        self.p = p
        # 在这里初始化你的 Generator
        # 请确保 ACVCGenerator 可以在此处被调用
    ...
    def apply_corruption(self, image_bgr):
        ...
        严格模仿离线代码的处理流程
        Args:
            image_bgr: NumPy BGR 格式图片
        Returns:
            image_bgr_corrupted: 处理后的 NumPy BGR 图片
        """
        # 1. 转换为 RGB
        img_rgb = image_bgr[:, :, ::-1]
        ...
        try:
            ...
            # 5. 转回 BGR
            return np.ascontiguousarray(aug_result[:, :, ::-1])

        except Exception as e:
            LOGGER.warning(f"[Warning] Corruption {c_type} failed: {e}")
            # 如果出错，返回原始图像
            return image_bgr
    ...
    def __call__(self, labels):
        """
        Args:
            labels (dict): 包含 'img' (numpy array) 等信息的字典
        """
        # 根据概率 p 决定是否增强
        if random.random() < self.p:
            # 修改 labels 中的 img
            labels["img"] = self.apply_corruption(labels["img"])
        return labels
